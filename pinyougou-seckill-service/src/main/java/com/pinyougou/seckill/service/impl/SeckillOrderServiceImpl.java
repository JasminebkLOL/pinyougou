...
public class SeckillOrderServiceImpl implements SeckillOrderService {
	...
	public void submitOrder(Long seckillId, String userId) {
		...
		redisTemplate.boundHashOps("seckillGoods").put(seckillId, seckillGoods);// 修改库存后再放回缓存
		// 如果已经被秒光,redis中需要删除库存为0的key:seckillId对应的seckillGoods对象
		if (seckillGoods.getStockCount().intValue() == 0) {// 如果已经被秒光
		...
		seckillOrder.setId(orderId);// 订单主键,唯一流水号:out_trade_no
		seckillOrder.setCreateTime(new Date());
		seckillOrder.setMoney(seckillGoods.getCostPrice());// 秒杀价格
		seckillOrder.setSeckillId(seckillId);// 订单秒杀的商品
		seckillOrder.setSellerId(seckillGoods.getSellerId());// 商品所属商家/````````````````````````
		seckillOrder.setUserId(userId);// 设置用户ID
		seckillOrder.setStatus("0");// 订单支付状态
